...
export default {
  getToken: async () => {return await AsyncStorage.getItem('token');},
  validateToken: async () => {
  ...
  addWarning: async (title, list) => {
    ...
    let json = await request('post', '/warning', {
      title, list, property: property.id,
    }, token);
  ...
  setReservation: async (id, date, time) => {
    ...
    let property = await AsyncStorage.getItem('property');
    property = JSON.parse(property);
    let json = await request('post', `/reservation/${id}`, {property: property.id, date, time}, token);
    return json;
  },
  ...
  setRecovered: async id => {
    let token = await AsyncStorage.getItem('token');
    let json = await request('put', `/foundandlost/${id}`, {status: 'recovered'}, token);
    return json;
  },
